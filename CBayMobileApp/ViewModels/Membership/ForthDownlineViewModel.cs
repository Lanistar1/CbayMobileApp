...
namespace CBayMobileApp.ViewModels.Membership
{
    ...
    public class ForthDownlineViewModel : BaseViewModel
    {
        public ForthDownlineViewModel(INavigation navigation, List<DownlineData> selectedItems)
        {
        ...
        private async Task GetTappedExecute(DownlineData model)
        {
            try
            {
                ...
                await Navigation.PushAsync(new FifthDownline(SelectedItems), true);
            }
            catch (Exception ex)
            {
